...
beta  = @(t) 0.5 + (t - R).*sqrt(R^2 - (t - R).^2)/(pi*R^2) + asin((t - R)/R)/pi;

for option = {'integral', 'function'}
    %% Approximate kernel
    % Order
    M = 2e1;
    ...
    switch lower(option{1})
        case 'integral'
            ...
            chat = identify_kernel_theoretical(beta, dt, M, option{1});
        case 'function'
            % Identify kernel for fixed order
            chat = identify_kernel_theoretical(alpha, dt, M, option{1});
    end
    ...
    % Save plot
    SavePlot(['beta_mixture_approximation_overview_', option{1}], SaveFigures, Format);
    ...
    % Save plot
    SavePlot(['beta_mixture_approximation_error_', option{1}], SaveFigures, Format);
end